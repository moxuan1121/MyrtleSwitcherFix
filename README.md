...
- `0.5.4.5~nokeyboard1`：独立分支删除键盘出现时点击窗口外关闭分屏的整条实现，包括精确 `TapOutsideAction` 取值 Hook、Myrtle 外部动作门控、宿主窗口 `hitTest:` Hook、键盘所有者状态及安装重试条件；固定键盘避让高度保留。
- `0.5.4.5`：修正 0.5.4.4 方向回调可能运行在 FrontBoard XPC 工作线程的问题；scene 方向更新保留原调用队列，所有 UIWindow 几何恢复与裁剪严格转交主线程，避免横屏打开时触发 CoreAnimation 非主线程事务终止。
- `0.5.4.4`：仅在 Myrtle 1.4.1 已确认的分屏 scene 方向重排入口固定承载应用为竖屏，后方主应用仍跟随系统横屏；窗口裁剪与命中测试统一使用当前屏幕坐标空间，使横屏窗口外触摸继续传递给后方应用，并避免返回竖屏时残留半截窗口。
- `0.5.4.3`：横屏时停用只适用于竖屏坐标的 WindowServer 窗口裁剪，完整保留 Myrtle 原生窗口几何；每次恢复原生几何后丢弃旧快照，防止横屏失败留下半屏裁剪并污染下一次竖屏打开。键盘窗口外关闭逻辑保持不变。
